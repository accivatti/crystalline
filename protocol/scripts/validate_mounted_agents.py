#!/usr/bin/env python3
# [INPUT]: reads agents/*.agent.md yaml blocks and their base role declarations;
#          checks role, tenant attachment, playbook, work_substrate, entrypoints,
#          run-state, and proactive GEB references.
# [OUTPUT]: mounted agent assembly validator; returns 0 on pass, 1 on first error.
# [POS]: scripts mounted-agent validator — checks assembled agent is wired to role,
#        tenant, work_substrate, state ledger, proactive learning gate, and that
#        mount playbooks ⊆ role playbooks (subset, not equal; empty role declaration
#        skips surface check with a note); no tenant special-casing; runtime not in
#        protocol scope.
...
def validate_agent(path: Path) -> None:
    ...
    if not isinstance(geb_learning.get("runtime_must_report"), list) or not geb_learning["runtime_must_report"]:
        raise ContractError(f"{path}: geb_learning.runtime_must_report must be non-empty")
